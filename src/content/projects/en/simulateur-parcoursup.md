...
- **Parcoursup Sheets (Training Programs)**
  Retrieval: presentation, _expectations_ (in % when available), baccalaureate distribution, milestones (candidates/admitted/admitted), tips, `competition` detection.
...
### a) Cleaning and Merging — _Training Programs_
...
4. **Adding "Competition"**
   ...
   Adding `competition` and `dossier_coeff`, `competition_coeff` from training programs present in certain competitions (_Sésame, Accès, Geipi-Polytech, Avenir, Advance, Puissance Alpha, IEP-Sciences Po_).

### b) Specialties

1. **Open Data**

   Filtering by year, pairs (doublets) → stable IDs (1..13), calculation of `admitted_specialty_share` per training program, intra-training _ranking_.
